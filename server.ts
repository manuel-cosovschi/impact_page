...
try {
  // Use in-memory DB on Vercel to avoid read-only filesystem errors
  const dbPath = process.env.VERCEL ? ":memory:" : "impact.db";
...
// Health check
app.get("/api/health", (req, res) => res.json({ status: "ok", env: process.env.NODE_ENV, db: !!db, vercel: !!process.env.VERCEL }));
...
const startServer = async () => {
  if (process.env.NODE_ENV !== "production") {
    ...
    // In production (or Vercel), we might want to serve static files
    // But on Vercel, static files are served by Vercel's edge.
    // We only serve them here if running as a standalone Node app.
...
// In Vercel, this file is imported, so this block won't run

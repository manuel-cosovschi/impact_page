    ...
    <div className="bg-[#020617] text-slate-100 font-sans min-h-screen selection:bg-sky-400 selection:text-slate-900">
      ...
      <main className="pt-32 pb-20 px-6">
        <div className="max-w-[1100px] mx-auto space-y-24">
            ...
            >
              <div className="flex flex-wrap gap-3 mb-6">
                {[
                  ...
                  { icon: Lightbulb, text: 'Interés en producto y UX' }
                ].map((badge, i) => (
          ...
          <section id="proyectos">
            <div className="flex items-end justify-between mb-10">
              <div>
                ...
                <p className="text-slate-400">Aprendizajes concretos en proyectos académicos y prácticas.</p>
              </div>
          ...
          {/* Impact Section */}
          <section id="enfoque" className="bg-gradient-to-br from-[#0f172a] to-[#0b1120] rounded-2xl p-1 border border-white/10 shadow-2xl">
            <div className="bg-[#020617]/80 backdrop-blur-3xl rounded-xl p-8 md:p-12">
              <div className="text-center mb-12">
                <h2 className="text-3xl font-bold text-white mb-3">Mi proceso de aprendizaje</h2>
                <p className="text-slate-400">Cómo vengo aprendiendo a resolver problemas técnicos.</p>
              </div>
              
              <div className="grid md:grid-cols-3 gap-6 mb-12">
                {[
                  { icon: Smartphone, title: 'Optimización', sub: 'Iteración Continua' },
                  { icon: Cpu, title: 'Escalabilidad', sub: 'Diseño Robusto', active: true },
                  { icon: Layout, title: 'UX / UI', sub: 'Usabilidad' }
                ].map((card, i) => (
                  <button 
                    key={i}
                    className={`p-8 rounded-xl group transition-all text-center border ${card.active ? 'bg-[#0f172a] border-sky-400 shadow-[0_0_30px_rgba(56,189,248,0.1)]' : 'bg-[#0f172a] border-white/5 hover:border-sky-400/50'}`}
                  >
                    <div className={`size-14 rounded-full flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform ${card.active ? 'bg-sky-400 text-slate-900' : 'bg-sky-400/10 text-sky-400'}`}>
                      <card.icon size={28} />
                    </div>
                    <h3 className="text-white font-bold text-lg mb-1">{card.title}</h3>
                    <p className={`text-xs ${card.active ? 'text-sky-400' : 'text-slate-500'}`}>{card.sub}</p>
                  </button>
                ))}
              </div>

              <div className="bg-[#0b1120] rounded-xl p-8 border border-white/5 font-mono text-sm shadow-inner">
                <div className="flex items-center gap-2 text-sky-400 mb-6">
                  <Terminal size={16} />
                  <span>ejecutando solucion_automatizada.sh...</span>
                </div>
                <div className="space-y-5">
                  {[
                    { step: 'Paso 1', text: 'Análisis del flujo actual y detección de puntos de fricción manual.' },
                    { step: 'Paso 2', text: 'Prototipado rápido de script para procesar datos y validar lógica.' },
                    { step: 'Paso 3', text: 'Refactorización e implementación de pruebas para asegurar consistencia.' }
                  ].map((item, i) => (
                    <div key={i} className="flex gap-6">
                      <span className="text-slate-600 w-16 shrink-0">{item.step}:</span>
                      <span className="text-slate-300">{item.text}</span>
                    </div>
                  ))}
                  <div className="flex gap-6 mt-8 pt-6 border-t border-white/10">
                    <span className="text-sky-400 font-bold w-16 shrink-0">Meta:</span>
                    <span className="text-white font-bold">Resultado esperado: reducción de trabajo manual y errores mediante iteración.</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <section id="adn">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
              {[
                { id: '01', title: 'Curiosidad', text: 'Me gusta entender el problema de fondo para proponer soluciones útiles.' },
                { id: '02', title: 'Resiliencia', text: 'Veo los errores como parte del aprendizaje y busco mejorar con cada uno.' },
                { id: '03', title: 'De Corazón', text: 'Intento acordarme de que del otro lado hay una persona. Cuando diseño o programo, me importa que sea claro, útil y fácil de usar — y también fácil de mantener para el equipo.' },
                { id: '04', title: 'Interés en IA aplicada', text: 'En FitNow exploré un módulo de recomendaciones con Ridge Regression y aprendí sobre regularización y estabilidad con pocos datos. Me interesa IA aplicada, y quiero seguir profundizando con mentores y casos reales.' }
              ].map((item) => (
